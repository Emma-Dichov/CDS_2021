...
author: "Emma Dichov Juul Rasmussen"
...
```{r}
Western_Europe <- read_csv("HomicideHistory-main/data/homicide-rates-across-western-europe.csv") 
...
To rename the column 'Western_Europe$`Homicide rate in Europe over long-term (per 100,000)' to 'homicides' i used the names-function to find the names in the dataset ‘Western_Europe’ and then I specified in the brackets which variable (column) I wanted it to extract and then I assigned it a new name ‘Homicides’ with the '<-': 

```{r, label='relabeling a column'}
...
I have choosen to make a line graph: 

```{r}
ggplot(data = Western_Europe, aes(x = Year, y = Homicides, color = Entity)) +
  geom_line(size=1.2)
...
To change the facet layout I used the facet_wrap function to create multiple subplots organized. I specified within the facet_wrap function that I wanted it organized into two columns and three rows with 'nrow' and 'ncol':
...
To move the legend to a position below the facets I used the theme function to specify the position of the legend to be at the bottom. To change the legends label from "Entity" to "country" I used the labs function where I specified/argued 'color = "Country". I had to use this command instead of 'fill = "New title"', because the lab argument had to match the argument I used in the aes function, which is 'Color":  

```{r}
ggplot(data = Western_Europe, aes(x = Year, y = Homicides, color = Entity)) +
  geom_line(size=1.2) +
  facet_wrap(~ Entity, nrow = 3) +
  labs(x = "Year", y = "Homicides per 100,000", title = "Homicide rate in Western Europe from 1300-2000", color = "Country") +
...
To add a floating table of contents in the header I used to toc option, where I specified that the depth 'toc_depth' and that the TOC should be floating 'toc_float'. To create chunk-names I used the argument 'label='title'' in the '{r}' under the code chunk, so '{r, label='relabeling a column'}, but I have been unable to get it to print the lable of the R-chunks in the html-file. Finally, to get it to generate a timestamp to show when it was last updated I used the sys.time()-function in the header. I further customized the format of the date to be 'DD/MM/YY'. So, in the header under date used this expression: "Last updated on `r format(Sys.time(), '%d %B, %Y')`" to create a timestamp. 
